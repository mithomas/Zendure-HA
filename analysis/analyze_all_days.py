...
def print_row_details(r, indent="    "):
    ...
    if prim == "wz_balkon":
        ...
        p_mode, p_sol, p_out, p_in_l, p_out_l, p_bat, p_state = r["wz_mode"], r["wz_sol"], r["wz_out"], r["wz_in_l"], r["wz_out_l"], r["wz_bat"], r["wz_state"]
        s_mode, s_sol, s_out, s_in_l, s_out_l, s_bat, s_state = r["k_mode"], r["k_sol"], r["k_out"], r["k_in_l"], r["k_out_l"], r["k_bat"], r["k_state"]
    else:
        prim_lbl, sec_lbl = "k_balkon(P)", "wz_balkon(S)"
        p_mode, p_sol, p_out, p_in_l, p_out_l, p_bat, p_state = r["k_mode"], r["k_sol"], r["k_out"], r["k_in_l"], r["k_out_l"], r["k_bat"], r["k_state"]
        s_mode, s_sol, s_out, s_in_l, s_out_l, s_bat, s_state = r["wz_mode"], r["wz_sol"], r["wz_out"], r["wz_in_l"], r["wz_out_l"], r["wz_bat"], r["wz_state"]
        
    print(f"{indent}Time: {r['time'].strftime('%H:%M:%S')} | Grid SML: {sml} W")
    print(f"{indent}Primary {prim_lbl}: Mode={p_mode}, State={p_state}, Solar={p_sol}W, Output={p_out}W (Limit={p_out_l}W), Bat={p_bat}W")
    print(f"{indent}Secondary {sec_lbl}: Mode={s_mode}, State={s_state}, Solar={s_sol}W, Output={s_out}W (Limit={s_out_l}W), Bat={s_bat}W")
...
def analyze_file(file_path):
    ...
    for p in import_periods[:3]:
        print(f"  {p['start'].strftime('%H:%M:%S')} to {p['end'].strftime('%H:%M:%S')} ({format_duration(p['duration'])}) - Avg: {p['avg_sml']:.1f}W")
    ...
    for sw in swings[:3]:
        print(f"  {sw['start_time'].strftime('%H:%M:%S')} to {sw['end_time'].strftime('%H:%M:%S')} - Magnitude: {sw['swing']:.1f} W ({sw['min_sml']:.1f} to {sw['max_sml']:.1f})")
